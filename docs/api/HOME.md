...
| data.records[].author.profileImageUrl | String | N | 작성자 프로필 이미지 URL (미설정 시 `null`) | `"/dummy/profile_01.png"` |
...
```json
{
  ...
  "data": {
    "records": [
      {
        "recordId": 42,
        "author": { "name": "박순자", "age": 68, "profileImageUrl": "/dummy/profile_01.png" },
        "title": "상추 모종 심을 땐 아침에만 물을 줘야 해요",
        "photoUrl": "/dummy/p1.png",
...
- 카드 탭 → 상세 페이지 이동은 `GET /api/v1/records/{recordId}` (DETAIL) 로 연결.
